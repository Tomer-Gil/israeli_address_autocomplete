...
const IS_COOKIE_HEADER = true;
const httpHeadersPath = "httpHeaders.txt";
...
const searchIsraelPost = async function(autoComplete) {
    ...
    let httpHeaders = await new Promise(function(resolve, reject) {
        fs.readFile(path.join(__dirname, httpHeadersPath), {encoding: "utf-8"}, function(err, data) {
            if(err) {
                reject("Error:\n" + err);
            } else {
                resolve(data);
    ...
    var israelPostResponse = await page.goto(LOCATE_ZIPCODE_URL);
    if(israelPostResponse._headers.hasOwnProperty("set-cookie") && israelPostResponse._headers["set-cookie"] !== httpHeaders.cookie) {
        ...
        try {
            let isHeadersFileUpdated = await new Promise(function(resolve, reject) {
                fs.writeFile(httpHeadersPath, JSON.stringify(httpHeaders), function(err) {
                    if(err) {
                        reject(err);
                    } else {
                        resolve(true);
        ...
        israelPostResponse = await page.goto(LOCATE_ZIPCODE_URL);
    }
